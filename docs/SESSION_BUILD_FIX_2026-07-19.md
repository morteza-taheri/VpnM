...
- **Remote Data Sources:** 
  - VPN Gate CSV API (`http://www.vpngate.net/api/iphone/`)
  - Mirror sites for fallback
...
### 4.1 Server List Sources

#### Primary Source
- **CSV API:** `http://www.vpngate.net/api/iphone/`
- Contains comprehensive server information including hostname, country, sessions, uptime, and OpenVPN configurations

#### Secondary Sources (Mirrors)
- Mirror sites listed at `https://www.vpngate.net/en/sites.aspx`
- Fallback mechanism when primary source is unavailable

### 4.2 Server Data Structure
The CSV contains the following columns:
- `#HostName`: Server hostname
- `CountryLong`: Full country name
- `CountryShort`: Country code
...
### 4.3 Server List Update
- **Automatic Update:** Every 15 minutes by default
...
- **Mirror Fallback:** Attempt mirror sites if primary source fails

## 5. Server List Interface & Controls
...
### 5.4 Server List Item Components
...
8. **Bookmark Star** toggle (explained in section 5.6)
...
- SoftEther: 🟢 (green chip)
- OpenVPN: 🔵 (blue chip)
- MS-SSTP: 🟠 (orange chip)
- L2TP/IPsec: 🟣 (purple chip)

**Badge Behavior:**
- Show all protocols supported by the server.
...
- **Calendar Integration:** 
  - Persian calendar for Persian language users
  - Gregorian calendar for English language users

### 6.4 Icon Requirements
...
- **Server List Updates:** Periodic WorkManager job (default 15 minutes)
